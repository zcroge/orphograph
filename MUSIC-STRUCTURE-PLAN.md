...
### P2 — Voicing derived from the word (owner's sub-question B) — SUPERSEDED, native design adopted instead

The sketch below (B.1–B.8) proposed replacing "flute pitch = harmonic of O by spoke-angle cosine, then table-voiced, then quantized" with a chord-voicing system built on real Western jazz/orchestration precedent: drop-2/drop-3 spread operators, low-interval-limit tables, rootless voicings, Tymoczko-style voice-leading-distance minimization, tertian-vs-quartal selection. Asked directly whether any of the project's musical-theoretical approach was arbitrary, the honest answer was that **this whole voicing layer was** — established by real precedent, but with no organic tie to the wheel's own geometry, unlike the letter-to-spoke mapping (grounded in place of articulation) or the ring-speed ratio (grounded in the Timaeus 6:8:12 proportion this project already treats as law). Kept, per direction: 12-TET and the transposition step's full reachable range including 7/"circle of fifths" (P0/P1, unaffected by this). Redesigned: the voicing layer itself, to be native — built from the SAME three vocabularies the rest of the piece already uses (spoke arc-distance, the 6:8:12 proportion, and the mirror/rotate transform operators), one level down, instead of borrowed jazz/orchestration categories. Landed in `src/voicing.js` (`voiceWord`), `synth.js`'s `meanderFlute`, and `main.js`'s `onNoteHit`/`onChordHit`.

**Content (was B.1, kept, already native).** `wordPitchClasses(word)` (main.js) — the word's own unique spokes, first-occurrence order. On each `onChordHit`, or at a word boundary in melody mode (`onNoteHit`, tracked per ring via `lastFluteWordByRing`), the flute's shared voice retunes to the new word's chord and sustains it until the next word boundary — not per letter, regardless of ring mode. All pitch goes through `hzForSpoke(transposedSpoke(spoke))`, same as the kalimba.

**Spacing (was B.3/B.4's spread-operator table, replaced).** For each pitch class besides the root, its own signed shortest-arc distance from the chord's root spoke (`signedArcDistance`, the same shortest-arc rule `wordArc`/`_directionToward` already use in sequencer.js, made pure) sets its octave offset from the root: `octaveOffset = sign(arcDistance) * floor(|arcDistance| / ARC_SPOKES_PER_OCTAVE)`, `ARC_SPOKES_PER_OCTAVE` defaulting to 2 (12 spokes / 6, the Timaeus proportion's own smallest member). A continuous function of the word's own geometry — a wide-arc word voices open, a close-arc word voices tight — replacing the six named drop-2/drop-3/tertian/quartal operators and their low-interval-limit table entirely. One tunable: `whistleVoicingArcSpokesPerOctave`.

**Register per ring (was B.4's register half, kept, already native).** `RING_OCTAVE_MULTIPLIER` (given lowest, made highest) still sets which octave the chord ROOT itself sits in; the arc-distance spacing above computes offsets from that root.

**Doubling from repeated letters (was B.6, replaced).** A letter that recurs within a word voices its second occurrence at the octave-offset mirrored through 0 from its first (its first sat `+n` octaves out → the echo sits `-n` out) — a real geometric echo via the same mirror law central to the whole piece, not "double the root, never the leading tone" (meaningless outside functional harmony).

**Rootless when the drone already holds it (was B.2, kept, reframed as native).** `|pcs| ≥ 3` omits the drone's own currently-held pitch class (found via the live transposition offset, not a fixed spoke); `|pcs| ≤ 2` keeps everything. Not actually jazz-specific — close to universal across polyphonic traditions, and here it follows directly from the drone's own law (it permanently holds the pole tone).

**Voice leading between words (was B.5's Tymoczko-distance minimization, replaced).** The single most foreign borrowing in the original sketch — "minimize total semitone motion" imports a distance metric from a completely different pitch-space tradition. Native replacement: if the next word's pitch-class SET is a mirror or rotation of the current word's (checked via the EXISTING `mirrorSpoke`/`rotateSpoke`, wheel.js), the next word's voicing applies that SAME operator to the current word's own realized voicing (each voice's octave offset carries over, its spoke transformed) — literal transform-consistency using the identical operator vocabulary that already governs mirror/rotate/retrograde at the whole-procession scale, one level down. Otherwise recomputed fresh via the spacing rule above.

**Retired:** B.7 (tertian-vs-quartal selector) and B.8's `whistleHarmonic`/`Min`/`Max`/the spoke-angle cosine sweep/the authored `whistleVoicing` table — all replaced by the rules above. `whistle-follow` off still falls back to a minimal single-oscillator manual pitch (`whistleManualRatio`), for auditioning timbre in isolation; `whistleScale`/`quantizeFluteHz` are live only on that fallback path now (skipped while `whistle-follow` is on, since a real letter's pitch needs no re-quantizing to a scale it's already the source of).

**Honest limit.** Structural verification (the arc-distance formula, the mirror-doubling and transform-voice-leading rules, the rootless behavior, and the word-boundary retune timing) confirms the rules are wired correctly. Whether the result actually sounds "rich and varied" versus the old table-based approach is a judgment only reachable by ear.
